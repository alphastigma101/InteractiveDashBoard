...
export class Library {
    ...
    addBook(author, details) { this.#Entries.set(author, details); }

    getLibrary() { return this.#Entries; }

    // Find book based of it's title
    findBook(name) {

        if (typeof name !== 'string') throw new TypeError(`Expected string for book name, got ${typeof name}`);

        for (const [author, book] of this.#Entries) {
            if (book.title === name) return true;
        }
    ...
    setLibrary(entry) {
        ...
        console.error("Invalid entry type. Expected Map.");
        return false;
    }

}

export const library = new Library(Book.Author, Book.Title, Book.Pages, Book.Read, Book.Published);
